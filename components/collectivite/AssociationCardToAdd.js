...
export default function AssociationCardToAdd({props, title, image, startDate}){
    const status = 1
        ...
        <>
            <Flex className="tw-mb-7">
                    ...
                    <Flex direction={'column'} ml={'lg'} justify={'space-around'}>
                        <Group mb={'sm'}>
                            <Avatar src={`/uploads/${props.association.avatar?.name}`} radius={'xl'} size={'sm'}/>
                            <Flex direction={'column'}>
                                <Text fz={'xs'} weight={800} className="tw-capitalize">{props.association.name}</Text>
                                <Text fz={'xs'} weight={600} className="tw-capitalize">{props.name}</Text>
                            </Flex>
                        </Group>
